...
在 Unity 中，切换到 SGen(Simple Generational Garbage Collector)可以通过修改启动参数来实现。SGen 是 Mono 的垃圾收集器之一，专为处理高性能和低延迟的场景而设计。
以下是切换到 SGen 的步骤：

## 1. 修改 Unity 的启动参数

你可以通过修改 Unity 编辑器的启动参数来启用 SGen。具体步骤如下：
Windows
...
```
"C:\Program Files\Unity\Editor\Unity.exe" -sgen
```

macOS

1. 打开终端
2. 使用以下命令启动 Unity 编辑器：

```sh
/Application/Unity/Unity.app/Contents/MacOS/Unity -sgen
```

## 2. 修改项目设置

你还可以通过修改项目的启动参数来启用 SGen。具体步骤如下：

1. 打开 Unity 项目。
2. 在菜单栏中选择`Edit->Project Settings`。
3. 在项目设置窗口中，选择 Player。
4. 在`Other Settings`部分，找到`Scripting Define Symbols`。
5. 添加以下符号：
...
要验证 SGen 是否已启用，可以在 Unity 编辑器中查看控制台输出。启动 Unity 编辑器后，打开控制台窗口（`Window->General->Console`），并查找与垃圾收集器相关的信息。
注意事项

- 性能测试：在切换到 SGen 后，务必进行性能测试，以确保它确实提高了你的项目性能。不同的项目可能会有不同的效果。
- Unity 版本：确保你使用的 Unity 版本支持 SGen。某些较旧的 Unity 版本可能不支持此功能。
- 平台支持：SGen 的支持可能因平台而异，请确保目标平台支持 SGen。

# IL2CPP

IL2CPP（Intermediate Language To C++）是 Unity 提供的一种脚本编译后端，它将 C#代码转换为 C++代码，然后编译为本机代码。IL2CPP 不使用 Mono 的垃圾收集器（包括 SGen），而是使用 Unity 自己的垃圾收集器。因此，无法在 IL2CPP 中切换到 SGen。

## IL2CPP 的垃圾收集器

IL2CPP 使用的是 Boehm-Demers-Weiser 垃圾收集器（Boehm GC），这是一个保守的垃圾收集器。Unity 选择 Boehm GC 是因为它在大多数情况下提供了良好的性能和稳定性。
